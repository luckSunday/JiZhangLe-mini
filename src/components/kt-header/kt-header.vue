  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      const agentInfo = computed(() => {
        return {
          code: `ID:${UserStore.getAgentId}-${UserStore.getSpreadId}`,
          userIsAgent: UserStore.user_agent_id && UserStore.user_agent_id === UserStore.getAgentId,
          hasAgentId: Boolean(UserStore.getAgentId),
        };
      });
      return {
        isRootPage,
...
<template>
  <view
    class="overflow-hidden header-container z-top"
  ...
  >
    ...
    >
      ...
      <template v-if="!customTitle">
        ...
        <!-- <view
        class="absolute bottom-0 left-0 flex min-w-[100px] items-end pl-10 opacity-0"
        :class="{ '!opacity-100': showAgentInfo || agentInfo.userIsAgent }"
        :style="{ color }"
        @longtap="showAgentInfo = !showAgentInfo"
      >
        {{ agentInfo.code }}
      </view> -->
      </template>
      <template v-else>
        <view
          class="flex items-center z-top"
          :class="[textAlign === 'center' && showLeftIcon ? 'text-center' : 'text-left']"
          :style="{ height: customStyle.height }"
        >
          <view
            class="flex-1 min-w-0 overflow-hidden text-base font-semibold overflow-ellipsis whitespace-nowrap z-top"
            :style="{ color }"
          >
            <slot></slot>
          </view>
...
<style lang="scss">
  .header-container {
    // .left-capsule {
    //   &.white {
    //     @apply border border-white border-opacity-20 bg-[#181627] bg-opacity-20;
    //   }
